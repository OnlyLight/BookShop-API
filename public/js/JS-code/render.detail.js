...
function render(items) {
	...
	if(items[0].soluongton == null) $('.mua').removeClass('hide');
	else
		if(items[0].soluongton < 1) $('.mua').addClass('hide');
	...
	for(var item of items) {
		...
		listDetail += '<br/><div class="link-col-right"><span>Tên Nhà Cung Cấp: '+item.tennhacc+' </span></div><hr/><div class="link-col-right"><span>Hình Thức: '+item.hinhthuc+'</span></div><hr/>';
		if(item.soluongton != null) {
			if(item.soluongton > 0) {
				listDetail += '<div class="link-col-right"><span>Tình Trạng: Còn hàng</span></div>';
			} else {
				listDetail += '<div class="link-col-right"><span>Tình Trạng: Hết hàng</span></div>';
			}
		} else {
			listDetail += '<div class="link-col-right"><span>Tình Trạng: Còn hàng</span></div>';
		}
	...
	$('#list-detail').html(listDetail + '<hr/>');
}
